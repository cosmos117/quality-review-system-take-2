"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.default = void 0;var _nodeFs = _interopRequireDefault(await jitiImport("node:fs"));
var _nodePath = _interopRequireDefault(await jitiImport("node:path"));
var _config = await jitiImport("prisma/config");function _interopRequireDefault(e) {return e && e.__esModule ? e : { default: e };}

// Prefer backend/.env for this repo, then fall back to root .env.
for (const relativePath of ["backend/.env", ".env"]) {
  if (process.env.DATABASE_URL) break;
  const absolutePath = _nodePath.default.resolve(process.cwd(), relativePath);
  if (_nodeFs.default.existsSync(absolutePath)) {
    process.loadEnvFile(absolutePath);
  }
}

if (!process.env.DATABASE_URL) {
  throw new Error(
    "DATABASE_URL is missing. Set it in backend/.env, .env, or your shell environment."
  );
}var _default = exports.default =

(0, _config.defineConfig)({
  schema: "prisma/schema.prisma",
  migrations: {
    path: "prisma/migrations"
  },
  datasource: {
    url: process.env.DATABASE_URL
  }
}); /* v9-ea8bfdfe4af38494 */
