...
export const createProject = async (req, res) => {
  try {
    ...
    const project = await projectService.createProject({
      project_no, internal_order_no, project_name, description,
      status, priority, start_date, end_date, created_by: creatorId,
      isReviewApplicable, reviewApplicableRemark,
    });
...
export const syncCheckpointCategories = async (req, res) => {
  try {
    ...
    res.status(200).json({ success: true, message: "Checkpoints synced with template categories" });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
};
...
export const getProjectStages = async (req, res) => {
  try {
    const stageData = await projectService.getProjectStages(req.params.projectId);
    const message = stageData.length === 0 ? "No stages found for this project" : "Project stages fetched successfully";
