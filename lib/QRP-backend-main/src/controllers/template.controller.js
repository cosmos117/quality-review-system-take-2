 ...
 * Accepts stage1, stage2, stage3, ... stage99 (NOT stage0)
 */
const isValidStage = (stage) => {
  return /^stage[1-9]\d?$/.test(stage);
};
...
const ensureTemplateConsistency = async (template) => {
  // No automatic stage creation - return false (not modified)
  return false;
};
...
export const getTemplate = asyncHandler(async (req, res) => {
  ...
  // Remove stage0 if it exists (legacy/buggy field) - filter out any stages that don't match valid format
  const validStageKeys = Object.keys(templateObj).filter((key) => /^stage[1-9]\d?$/.test(key));
  const cleanedObj = {};
  for (const key of validStageKeys) {
    cleanedObj[key] = templateObj[key];
  }
  // Copy non-stage fields
  for (const [key, value] of Object.entries(templateObj)) {
    if (!key.startsWith('stage') || /^stage[1-9]\d?$/.test(key)) {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, cleanedObj, "Template fetched successfully"));
});
...
export const deleteStageFromTemplate = asyncHandler(async (req, res) => {
  ...
  // Allow deletion of stage0 (legacy field) even though it doesn't pass validation
  if (stage !== 'stage0' && !isValidStage(stage)) {
    throw new ApiError(400, "Invalid stage format. Must be stage1-99");
  }
  ...
  // Get all available stages (including invalid ones for migration purposes)
