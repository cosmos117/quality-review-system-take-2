...
export const createTemplate = asyncHandler(async (req, res) => {
  ...
  if (template) {
    ...
    const stages = Object.keys(template.toObject()).filter((key) =>
      .status(200)
 ...
 * Body: { stage: "stage1" | "stage2" | ... | "stage99", text: "Checklist Name" }
 */
 ...
 * Removes a checklist group from the template (along with all its sections and questions)
...
export const deleteChecklistFromTemplate = asyncHandler(async (req, res) => {
  ...
  // Validate stage exists and is an array
  if (!Array.isArray(template[stage])) {
    throw new ApiError(404, `Stage ${stage} not found or has no checklists`);
  }

  const before = template[stage].length;

  // Find the checklist to verify it exists
  const checklistExists = template[stage].some(
    (item) => item._id.toString() === checklistId,
  );
  ...
  // Use MongoDB's native $pull operator to remove the checklist from the array
  // This is necessary because Mongoose's save() doesn't persist changes to dynamic fields properly
  ...
  const after = (updatedTemplate[stage] || []).length;

  return res
    .status(200)
    .json(
      new ApiResponse(200, updatedTemplate, "Checklist deleted successfully"),
    );
});
...
export const deleteCheckpointFromSection = asyncHandler(async (req, res) => {
  ...
  // Verify checkpoint exists in section
  const checkpointExists = section.checkpoints.some(
    (item) => item._id.toString() === checkpointId,
  );
  ...
  // Use MongoDB's $pull to remove checkpoint from nested section array
  const updateResult = await Template.collection.updateOne(
    { _id: template._id },
    {
      $pull: {
...
export const deleteCheckpointFromTemplate = asyncHandler(async (req, res) => {
  ...
  const checkpointExists = checklist.checkpoints.some(
    (item) => item._id.toString() === checkpointId,
  );
  ...
  // Use MongoDB's $pull to remove checkpoint from nested array
  const updateResult = await Template.collection.updateOne(
    { _id: template._id },
    {
      $pull: {
  ...
  return res
    .status(200)
    .json(
      new ApiResponse(200, updatedTemplate, "Checkpoint deleted successfully"),
    );
});
...
export const deleteSectionFromChecklist = asyncHandler(async (req, res) => {
  ...
  const sectionExists = (checklist.sections || []).some(
    (item) => item._id.toString() === sectionId,
  );
  ...
  // Use MongoDB's $pull to remove section from nested array
  const updateResult = await Template.collection.updateOne(
    { _id: template._id },
    {
      $pull: {
  ...
  return res
    .status(200)
    .json(
      new ApiResponse(200, updatedTemplate, "Section deleted successfully"),
    );
});
...
export const addStageToTemplate = asyncHandler(async (req, res) => {
  ...
  const updatedTemplate = await Template.findOne();
  const savedStagesInDb = Object.keys(updatedTemplate.toObject()).filter(
    (key) => /^stage\d{1,2}$/.test(key),
  );
