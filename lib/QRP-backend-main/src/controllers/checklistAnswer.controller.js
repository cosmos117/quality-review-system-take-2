...
const getSubmissionStatus = asyncHandler(async (req, res) => {
  ...
  // Get all answers for this project/phase/role
  const answers = await ChecklistAnswer.find({
    project_id: projectId,
    phase: phaseNum,
    role: normalizedRole,
  ...
  const isSubmitted =
    answers.length > 0 && answers.every((ans) => ans.is_submitted === true);
  const submissionCount = answers.filter((ans) => ans.is_submitted).length;
  ...
  return res
    ...
    .json(
      new ApiResponse(
        200,
        { is_submitted: isSubmitted, totalAnswers: answers.length, submissionCount },
