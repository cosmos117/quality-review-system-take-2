...
const getChecklistAnswers = asyncHandler(async (req, res) => {
  ...
  // Fetch all checkpoints for this project/phase to get category/severity
  const checkpoints = await Checkpoint.find({
    project_id: projectId,
    phase: phaseNum,
  }).select("sub_question defect");

  // Create a map of sub_question -> checkpoint data
  const checkpointMap = {};
  checkpoints.forEach((cp) => {
    if (cp.sub_question) {
      checkpointMap[cp.sub_question] = {
        categoryId: cp.defect?.categoryId || "",
  ...
  answers.forEach((ans) => {
    ...
    // Include categoryId and severity from the answer itself (primary source)
    if (ans.categoryId) {
      answerData.categoryId = ans.categoryId;
    }
    if (ans.severity) {
      answerData.severity = ans.severity;
    }

    // Fallback: try to get from checkpoint if not in answer (for backward compatibility)
    if (!answerData.categoryId || !answerData.severity) {
      const checkpointData = checkpointMap[ans.sub_question];
      if (checkpointData) {
        if (!answerData.categoryId && checkpointData.categoryId) {
          answerData.categoryId = checkpointData.categoryId;
        }
        if (!answerData.severity && checkpointData.severity) {
          answerData.severity = checkpointData.severity;
        }
      }
    }
...
const submitChecklistAnswers = asyncHandler(async (req, res) => {
  ...
  // If executor is submitting after a revert, clear the "reverted_to_executor" status
  // and also clear the reviewer's submission so they can review the updated work again
  if (normalizedRole === "executor") {
    const approval = await ChecklistApproval.findOne({
      project_id: projectId,
      phase: phaseNum,
    });

    if (approval && approval.status === "reverted_to_executor") {
      // Clear the approval status
      await ChecklistApproval.updateOne(
        { project_id: projectId, phase: phaseNum },
        { $unset: { status: "", decided_at: "", decided_by: "", notes: "" } },
      );

      // Clear reviewer's submission so they can review again
      await ChecklistAnswer.updateMany(
        { project_id: projectId, phase: phaseNum, role: "reviewer" },
        { $set: { is_submitted: false, submitted_at: null } },
      );

      console.log(
        `🔄 Executor resubmitted after revert - cleared status and reviewer submission for phase ${phaseNum}`,
      );
    }
  }
