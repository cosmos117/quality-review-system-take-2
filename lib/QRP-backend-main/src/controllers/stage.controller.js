...
const deriveStageKey = (stageName = "") => {
  ...
  if (lower.includes("phase 1") || lower.includes("stage 1")) return "stage1";
  if (lower.includes("phase 2") || lower.includes("stage 2")) return "stage2";
  if (lower.includes("phase 3") || lower.includes("stage 3")) return "stage3";
  return null;
};
...
const cloneTemplateToProject = async (projectId, userId) => {
  ...
  for (const key of stageKeys) {
    const stageName = key === "stage1" ? "Phase 1" : key === "stage2" ? "Phase 2" : "Phase 3";
    const stage = await Stage.create({
      project_id: projectId,
      stage_name: stageName,
      stage_key: key, // Store the template stage key
      status: "pending",
      created_by: creatorId,
    });
    stageDocs.push({ doc: stage, key });
  }
  ...
  return stageDocs.map((s) => s.doc);
};

/**
 * LIST STAGES FOR PROJECT
 * GET /api/v1/projects/:projectId/stages
 * Fetches all stages for a project, cloning from template if needed
 */
export const listStagesForProject = asyncHandler(async (req, res) => {
  ...
  let stages = await Stage.find({ project_id: projectId }).sort("stage_name");
 ...
 */
export const getStageById = asyncHandler(async (req, res) => {
  const { id } = req.params;
 ...
 * Creates a new stage (manual creation)
 */
 ...
 */
export const incrementLoopbackCounter = asyncHandler(async (req, res) => {
