...
const listStagesForProject = asyncHandler(async (req, res) => {
  ...
  // Debug: Log stage counters being returned
  console.log(`📊 Returning ${stages.length} stages for project ${projectId}:`);
  stages.forEach((stage) => {
    console.log(
      `  Stage (${stage.stage_key}): loopback_count=${stage.loopback_count}, conflict_count=${stage.conflict_count}`
    );
  });
  
  // Additional debug: Log the actual JSON that will be sent
  console.log(`📤 JSON Response sample (first stage):`, JSON.stringify(stages[0], null, 2));
