...
export const getProjectMembers = async (req, res) => {
    try {
        ...
        console.log(`[ProjectMembership] Getting members for project ${project_id}`);
        
        // Check if project exists
        const project = await Project.findById(project_id);
        if (!project) {
            console.log(`[ProjectMembership] Project ${project_id} not found`);
        ...
        const validMembers = members.filter(m => {
            if (!m.user_id || !m.user_id._id) {
                console.log(`[ProjectMembership] Skipping membership with invalid user_id:`, m._id);
                return false;
            }
            if (!m.role || !m.role._id) {
                console.log(`[ProjectMembership] Skipping membership with invalid role:`, m._id);
                return false;
            }
            return true;
        });
        
        console.log(`[ProjectMembership] Found ${validMembers.length} valid members (${members.length} total, ${members.length - validMembers.length} filtered)`);
        ...
        console.error(`[ProjectMembership] Error getting members:`, error);
        res.status(500).json({
            success: false,
