...
const getProjectChecklist = asyncHandler(async (req, res) => {
  ...
  // Calculate current defects for each group (current iteration mismatches)
  ...
  checklistObj.groups = checklistObj.groups.map((group) => {
    ...
    return {
      ...group,
      currentDefects, // Current mismatches in ongoing iteration
      // defectCount remains as historical/cumulative value
    };
  });
  ...
  return res
    ...
    .json(
      new ApiResponse(
        200,
        checklistObj,
        "Project checklist fetched successfully",
      ),
    );
});
 ...
 *
 * Formula: Sum of defect rates (with defects only) / Count of iterations with defects
 * Note: Iterations with 0% defect rate are excluded from both sum and count
 *
 * Iteration Lifecycle:
 * - Iterations are saved to checklist.iterations array when reviewer REVERTS to executor
 * - When reviewer APPROVES, no iteration is saved (final iteration not in array)
 * - Current working iteration is in checklist.groups (not in iterations array yet)
 * - We count: past iterations (from array) + current iteration (if new and has defects)
 */
const getOverallDefectRate = asyncHandler(async (req, res) => {
  ...
  // For new calculation: collect all iteration defect rates
  ...
  // Helper function to calculate defect rates for all iterations in a phase
  const calculateIterationDefectRates = (checklist) => {
    ...
    for (let i = 0; i < checklist.iterations.length; i++) {
      ...
      // Calculate NEW defects in this iteration only (not cumulative)
      const newDefectsInIteration =
        cumulativeDefects - previousIterationDefects;
      previousIterationDefects = cumulativeDefects;
      ...
      // Cap iteration defect rate at 100% (defects can't exceed total questions)
  ...
  for (const stage of stages) {
    ...
    if (checklist) {
      ...
      // Calculate CURRENT defects for this phase (current iteration only)
      ...
      // NEW: Calculate defect rates for all PAST iterations in this phase
      ...
      // Add ONLY non-zero past iteration defect rates to the sum
      // We ONLY count COMPLETED iterations (those saved in the iterations array)
      // Current work-in-progress is NOT counted until it's saved to iterations array
      ...
      console.log(
        `After phase ${stage.stage_key}: Sum = ${sumOfAllIterationDefectRates}, Total iterations (with defects) = ${totalIterationsAcrossAllPhases}`,
      );
    }
  }

  // FORMULA: Overall defect rate = Sum of all iteration defect rates / Total number of iterations
  const divisor =
    totalIterationsAcrossAllPhases > 0 ? totalIterationsAcrossAllPhases : 1;

  let overallDefectRate =
    divisor > 0
      ? parseFloat((sumOfAllIterationDefectRates / divisor).toFixed(2))
      : 0;

  // Cap at 100% as defect rate should never exceed 100%
  if (overallDefectRate > 100) {
    overallDefectRate = 100;
  }

  // Store the calculated overall defect rate in the project
