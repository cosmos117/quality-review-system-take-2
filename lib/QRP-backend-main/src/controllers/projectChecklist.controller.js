...
const getOverallDefectRate = asyncHandler(async (req, res) => {
  ...
  const startedPhaseDefectRates = []; // Track defect rates only for started phases
  ...
  // Helper function to check if phase has started (current iteration)
  const hasPhaseStarted = (checklist) => {
    // A phase has started if there are any answered questions in current iteration
    for (const group of checklist.groups) {
      if (group.questions && Array.isArray(group.questions)) {
        for (const q of group.questions) {
          if (
            (q.executorAnswer !== null && q.executorAnswer !== undefined) ||
            (q.reviewerAnswer !== null && q.reviewerAnswer !== undefined)
          ) {
            return true;
          }
        }
  ...
  for (const stage of stages) {
    ...
    if (checklist) {
      ...
      // The current defect rate should only reflect defects in the ongoing iteration
      ...
      // Only add to startedPhaseDefectRates if phase has started
      if (hasPhaseStarted(checklist)) {
        startedPhaseDefectRates.push(phaseDefectRate);
      }
    }
  }

  // Calculate overall defect rate as average of started phase defect rates
  // This ensures overall rate = sum of individual phase rates / number of started phases
  const overallDefectRate =
    startedPhaseDefectRates.length > 0
      ? parseFloat(
          (
            startedPhaseDefectRates.reduce((sum, rate) => sum + rate, 0) /
            startedPhaseDefectRates.length
          ).toFixed(2),
        )
      : 0;
