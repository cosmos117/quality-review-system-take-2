...
const updateExecutorAnswer = asyncHandler(async (req, res) => {
  ...
  if (imagesToDelete.length > 0) {
    try {
      ...
      console.log(
        `[Executor] Deleted ${imagesToDelete.length} removed images from GridFS`,
      );
    } catch (error) {
      console.error("[Executor] Error deleting images from GridFS:", error);
...
const updateReviewerStatus = asyncHandler(async (req, res) => {
  ...
  if (imagesToDelete.length > 0) {
    try {
      await deleteImagesByFileIds(imagesToDelete);
      console.log(
        `[Reviewer] Deleted ${imagesToDelete.length} removed images from GridFS`,
      );
    } catch (error) {
      console.error("[Reviewer] Error deleting images from GridFS:", error);
      // Don't fail the request if image deletion fails
    }
  }
