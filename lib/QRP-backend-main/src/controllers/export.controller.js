...
export const exportMasterExcel = asyncHandler(async (req, res) => {
  try {
    ...
    employeeSheet.columns = [
      ...
      { width: 35 }, // Email
      { width: 20 }, // Completed Projects
      { width: 20 }, // Ongoing Projects
      { width: 25 }, // Total Assigned Projects
      { width: 22 }, // Average Defect Rate (%)
    ];

    // Create employee performance map
    const employeePerformanceMap = new Map();

    // Loop through all memberships to aggregate employee data
    memberships.forEach((membership) => {
      ...
      if (!employeePerformanceMap.has(userId)) {
        employeePerformanceMap.set(userId, {
          ...
          ongoingProjects: new Set(),
          totalProjects: new Set(),
          teamLeaderProjects: [], // Store projects where user is team leader
        });
      }
      ...
      // Check project status - count all non-completed projects as ongoing
      const status = project.status?.toLowerCase() || "";
      if (status === "completed") {
        empData.completedProjects.add(projectId);
      } else {
        // Any status other than completed is considered ongoing
        empData.ongoingProjects.add(projectId);
      }

      // Track projects where this user is a team leader
      if (roleName === "TeamLeader" && !empData.teamLeaderProjects.some(p => p.projectId === projectId)) {
        empData.teamLeaderProjects.push({
          projectId: projectId,
          defectRate: project.overallDefectRate,
        });
      }
    });
    ...
    employeePerformanceMap.forEach((empData) => {
      // Calculate average defect rate for projects where employee is team leader
      const avgDefectRate = empData.teamLeaderProjects.length > 0
        ? (
            empData.teamLeaderProjects
              .filter(p => p.defectRate !== null && p.defectRate !== undefined)
              .reduce((sum, p) => sum + p.defectRate, 0) / 
            empData.teamLeaderProjects.filter(p => p.defectRate !== null && p.defectRate !== undefined).length
          ).toFixed(2)
        : "";

      employeeSheet.addRow([
        safeValue(empData.name),
        safeValue(empData.email),
        empData.completedProjects.size,
        empData.ongoingProjects.size,
        empData.totalProjects.size,
        safeValue(avgDefectRate),
      ]);
    });

    // ===== Process each project =====
    for (const project of projects) {
      // Skip projects that are not applicable for review
      if (project.isReviewApplicable === "no") {
        continue;
      }
      ...
      const isReviewApplicable =
        ...
        project.isReviewApplicable === undefined
          ? ""
          : project.isReviewApplicable === "yes"
            ? "Yes"
            : "No";

      const overallDefectRate =
        project.overallDefectRate !== null &&
        project.overallDefectRate !== undefined
    ...
    // ===== Sheet 3: Review Not Applicable Projects =====
    ...
    notApplicableHeaderRow.eachCell((cell) => {
      ...
      cell.fill = {
        type: "pattern",
        pattern: "solid",
        fgColor: { argb: "FFE74C3C" }, // Red color for not applicable
      };
      cell.alignment = {
        horizontal: "center",
        vertical: "center",
    ...
    // Filter and add projects where isReviewApplicable === 'no'
    const notApplicableProjects = projects.filter(
      (p) => p.isReviewApplicable === "no"
    );
    ...
    for (const project of notApplicableProjects) {
      ...
      const statusDisplay = 
        ...
        project.status === "pending" ? "Not Started" :
        project.status;

      // Map priority for display
      ...
      notApplicableSheet.addRow([
        ...
        safeValue(statusDisplay),
        safeValue(priorityDisplay),
        safeValue(project.created_by?.name || project.created_by?.email || ""),
