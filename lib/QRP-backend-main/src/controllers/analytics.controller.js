...
export const getProjectAnalysis = asyncHandler(async (req, res) => {
  ...
  checkpoints.forEach((checkpoint) => {
    ...
    if (checklist) {
      ...
      defectsByPhase[stageId].totalCheckpoints++;
      defectsByChecklist[checklistId].totalCheckpoints++;

      if (checkpoint.defect.isDetected) {
        defectsByPhase[stageId].defectCount++;
        defectsByChecklist[checklistId].defectCount++;
...
export const getCategoryDistribution = asyncHandler(async (req, res) => {
  ...
  // Filter only defected checkpoints
  const defectedCheckpoints = checkpoints.filter(
    (cp) => cp.defect && cp.defect.isDetected
  );
