  ...
  try {
    ...
    if (prevStatus === "pending" && existing.status === "in_progress") {
      ...
      if (existingStagesCount === 0) {
        ...
        console.log(
          "⚠️ Stages already exist for this project, skipping creation",
        );
      }
    }
  ...
  try {
    ...
    try {
      ...
      if (uniqueFileIds.length > 0) {
        ...
        console.log(
          `[Project Delete] Deleted ${uniqueFileIds.length} images from GridFS`,
        );
      }
    } catch (imageError) {
      console.error("[Project Delete] Error deleting images:", imageError);
