...
export const assignDefectCategory = asyncHandler(async (req, res) => {
  ...
  // Only assign category if a defect exists
  if (!checkpoint.defect.isDetected) {
    throw new ApiError(400, "Cannot assign category: no defect detected for this checkpoint");
  }
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, checkpoint, "Defect category assigned successfully"));
});
